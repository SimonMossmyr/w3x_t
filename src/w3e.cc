...
w3e_type w3e_to_struct(string contents) {
    ...
    for (int i = 0; i < number_of_tilepoints; i++) {
        ...
        w3e.tilepoint[i].water_level = water_level_and_boundary_flag & 0x7FFF; // lowest 15 bits
        w3e.tilepoint[i].flag.boundary_1 = (bool)(water_level_and_boundary_flag & 0x8000); // highest bit;
